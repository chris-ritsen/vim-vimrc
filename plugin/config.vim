...
let g:NERDTreeIgnore = ['node_modules', 'jspm_packages', '\.xkm$', '\.o$', '\.fixed\.svg$', '\.git/', '\.csproj.user$', '\.csproj.vspscc$', '.ico$', '.suo$', '.png$', '.min.js$', '.vssscc$', '.min.map$', '.cache$', '.dll$', '.pdb$', '.nupkg$', '.nuspec$', '.gif$', '.eot$', '.ttf$', '.otf$', '.woff$', '.wav$', '.mp3$', 'obj.*', 'bin.*', '3rdParty']
